...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[rptMianExamOverallPassFailStatistic]'))
BEGIN
DROP PROCEDURE  rptMianExamOverallPassFailStatistic
END
GO
SET ANSI_NULLS ON
GO
...
GO
-- execute [rptMianExamOverallPassFailStatistic] ' MainExamId IN (1,2,3) AND IsDeleted=0 ',1,9
CREATE PROCEDURE [dbo].[rptMianExamOverallPassFailStatistic]
(
...
-------Version & Session Wise Grade Summery Single--------------------------------------------------------------------------------------------------------------
	...
	(SELECT COUNT(StudentIID) FROM #MainExamResult R WHERE R.GradeLetter=G.GL AND StudentIID IN ( SELECT StudentIID FROM #STUDENTLIST l WHERE l.ClassId=C.ClassId ) )	 
	...
	WHERE G.SessionID=@SessionId and G.IsDeleted=0 AND G.ClassID IN(SELECT ClassId FROM #MainExamIds)
	...
	--------sET tOTAL
		...
		SET @ROW=@@ROWCOUNT
		 ...
		 BEGIN
			...
			SELECT @NoOfStudent=Count(*)  FROM #MainExamResult WHERE 
			StudentIID IN (SELECT StudentIID FROM #STUDENTLIST WHERE ClassId =(SELECT ClassId FROM #ClassWiseStudent WHERE ID=@INDEX))

			SELECT @NoOfPassStudent=Count(*)  FROM #MainExamResult WHERE IsPass=1 AND
			StudentIID IN (SELECT StudentIID FROM #STUDENTLIST WHERE ClassId =(SELECT ClassId FROM #ClassWiseStudent WHERE ID=@INDEX))
...
-------GPRangeWise--------------------------------------------------------------------------------------------------------------------------------
		...
		R.StudentIID,
		 GPA ,
		 (SELECT COUNT(StudentIID) FROM #MainExamResult WHERE IsPass=1 AND VersionID=S.VersionID AND ClassId=S.ClassId AND GroupId=S.GroupId AND SectionId=S.SectionId) AS PassStu
		...
		WHERE R.MainExamId IN (SELECT MainExamId FROM #MainExamIds)

		ORDER BY Category
-------GPRangeWise--------------------------------------------------------------------------------------------------------------------------------
	 
	   DROP TABLE #OverallStatiistics,#STUDENTLIST,#MainExamResult,#GradeWiseReport, #MainExamIds,#ClassWiseStudent
END


--SELECT * FROM Student_BasicInfo WHERE ClassId=4 AND IsDeleted=0 AND STATUS='A' ORDER BY RollNo
--SELECT * FROM Res_MainExamResult WHERE MainExamId= StudentIID=4601
