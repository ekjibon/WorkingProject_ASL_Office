...
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[GetMainExamAssessmentMarks]'))
BEGIN
DROP PROCEDURE  GetMainExamAssessmentMarks
END
GO
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE GetMainExamAssessmentMarks
(
@MainExamId int,
@SubjectId int,
@version int,
...
BEGIN
	...
	SELECT @SUBNAME=ISNULL([SubjectName],'') FROM [dbo].[Res_AssessmentSubject] WHERE AId=@SubjectId
		               SELECT
                              ...
                              B.StudentInsID,
                              CAST(B.RollNo AS INT) AS RollNo,
                              B.FullName,
							  ...
							  FROM  [Student_BasicInfo] AS B LEFT JOIN [Res_MainExamAssessment] AS A ON B.StudentIID=A.StudentID 
							  AND A.MainExamId = MainExamId AND A.SubjectId = @SubjectId
