...
IF EXISTS (SELECT *  FROM   sys.objects WHERE  object_id = Object_id(N'[Transactiondetailinsert_InvoiceCollection]')) 
  BEGIN 
      DROP PROCEDURE Transactiondetailinsert_InvoiceCollection 
  END 
go 
SET ansi_nulls ON 
go 
...
CREATE PROCEDURE Transactiondetailinsert_InvoiceCollection 
@FiscalYear        INT = NULL, 
@BranchId          INT , 
@Number            VARCHAR(20) ='', 
...
@LastCount           INT 
        ...
        INSERT INTO [dbo].[Acc_transaction] ([FiscalYearId], [AccBranchId],[TransNo],[TransType],[date],[drtotal],[crtotal],[isdeleted],[addby],[adddate],[remarks],[status],[ip],[macaddress],IsApproved,InvoiceTransType,CollectionMasterId)         
		VALUES     ( @FiscalYear,@BranchId, @Number, @Type, @Date, @DrTotal,@CrTotal,0,@AddBy,Getdate(),@Remarks,'Pending',@IP,@MAC,0 ,@InvoiceTransType,@CollectionMasterId) 
