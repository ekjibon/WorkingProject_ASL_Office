IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[rptGetBalanceSheet]'))
BEGIN
DROP PROCEDURE  rptGetBalanceSheet
END
GO
SET ANSI_NULLS ON 
...
GO
--rptGetBalanceSheet
Create PROCEDURE [dbo].rptGetBalanceSheet 
AS
BEGIN  
...
-(select SUM(led1.CurrentBalance) FROM dbo.ACC_Ledgers led1 where led1.RootGroupId=4 and led1.IsDisplay=1 and led1.IsLedger=1) as CurrentBalance ,0,0,0
	...
	--JOIN [dbo].[ACC_Transaction] T ON T.Id = Td.TransactionId
	  where led1.IsDisplay=1 and led2.IsDisplay=1 and led1.RootGroupId=1 and led1.IsLedger=1 --AND T.IsApproved =1 AND T.IsReject = 0 
