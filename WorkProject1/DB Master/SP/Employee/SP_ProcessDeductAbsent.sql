...
GO
/****** Object:  StoredProcedure [dbo].[SP_ProcessDeductAbsent]    Script Date: 3/30/2020 2:51:55 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
--  EXEC SP_ProcessDeductAbsent 1
ALTER PROCEDURE [dbo].[SP_ProcessDeductAbsent]
(
@PeriodID AS Int = 0
...
DECLARE @CategoryID AS INT
	...
	WHERE CONVERT(NVARCHAR(12),CAST(A.Intime AS DATE), 112) between @StartDate and @EndDate AND A.IsLeave=0 AND A.IsPresent=0 AND A.IsAbsetLongHoliday = 0 AND A.IsDutyOff = 0  AND EI.CategoryID=@CategoryID
	...
	WHILE @@FETCH_STATUS = 0
		BEGIN
			...
			SELECT  @PeriodID, 1008,@EmpId,ROUND((ISNULL(cast(CurrentSalary as decimal(18,2)),0)/30*@NoOfDays),0),getdate(),CategoryID,cast(@NoOfDays as nvarchar(50))+' days absent'
